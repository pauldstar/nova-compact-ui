<template>
    <section class="mb-8">
        <tabs
            ...
            :id="fieldName"
            :editable="$route.name !== 'custom-detail'"
            :sortable="$route.name !== 'custom-detail'"
...
export default {
    mixins: [ReactiveField, FormField, FormFieldStatus, ResourceCrud],
    ...
    mounted() {
        Nova.$on('action-executed', this.getTableGroups);
    ...
    destroyed() {
        Nova.$off('action-executed', this.getTableGroups);
    ...
    methods: {
        ...
        openRelationModal(resourceName, resourceId) {
            ...
            if (resourceName === 'table-notes') {
                // leave modalViaResource empty else nova won't save the table note
        ...
        async createGroup() {
            let newGroup = {
                ...
                groupable: this.resourceId,
                groupable_type: this.resourceName,
